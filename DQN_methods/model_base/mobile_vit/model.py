...
from model_base.mobile_vit.transformer import TransformerEncoder
from model_base.mobile_vit.model_config import get_config
...
class MobileViT_multi(nn.Module):
    """
    This class implements the `MobileViT architecture <https://arxiv.org/abs/2110.02178?context=cs.LG>`_
    """
    def __init__(self, model_cfg: Dict, num_classes = 100):
        super().__init__()
        ...
        self.linear = nn.Sequential(nn.Linear(exp_channels, self.num_classes),
                                    nn.Softmax(dim=1))
        # self.classifier.add_module(name="fc", module=nn.Linear(in_features=exp_channels, out_features=num_classes))
...
def mobile_vit_multi(num_classes=100):
    # pretrain weight link
    # https://docs-assets.developer.apple.com/ml-research/models/cvnets/classification/mobilevit_xxs.pt
    config = get_config("xx_small")
    m = MobileViT_multi(config, num_classes=num_classes)
    return m


if __name__ == '__main__':
    inputs = torch.rand(2, 5, 100, 100)
    model = mobile_vit_multi()
